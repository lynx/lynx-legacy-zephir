...
ZEPHIR_INIT_CLASS(Lynx_Cache_Memcached) {
	...
	ZEPHIR_REGISTER_CLASS_EX(Lynx\\Cache, Memcached, lynx, cache_memcached, lynx_cache_cachedriver_ce, lynx_cache_memcached_method_entry, 0);

	zend_declare_property_null(lynx_cache_memcached_ce, SL("options"), ZEND_ACC_PROTECTED TSRMLS_CC);

	zend_declare_property_null(lynx_cache_memcached_ce, SL("instance"), ZEND_ACC_PROTECTED TSRMLS_CC);
...
PHP_METHOD(Lynx_Cache_Memcached, setInstance) {
	...
	if (!(zephir_is_instance_of(instance, SL("Memcached") TSRMLS_CC))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STRW(spl_ce_InvalidArgumentException, "Parameter 'instance' must be an instance of 'Memcached'", "", 0);
		return;
	}
...
PHP_METHOD(Lynx_Cache_Memcached, getInstance) {
	...
	if (Z_TYPE_P(_0) == IS_NULL) {
		ZEPHIR_INIT_VAR(_1);
		_2 = zend_fetch_class(SL("Memcached"), ZEND_FETCH_CLASS_AUTO TSRMLS_CC);
...
PHP_METHOD(Lynx_Cache_Memcached, save) {
	...
	int ZEPHIR_LAST_CALL_STATUS;
	zval *id_param = NULL, *data, *lifeTime = NULL, *_0 = NULL;
	zval *id = NULL;
